...
<div class="container py-4">
    ...
    <ul class="nav nav-tabs mb-3">
        {{-- EN PROCESO --}}
        <a class="nav-link {{ request()->routeIs('checkouts.index') ? 'active' : '' }}" href="{{ route('checkouts.index', request()->only('edificio_id')) }}">
            🔄 En proceso
        </a>

        {{-- CERRADOS --}}
        <a class="nav-link {{ request()->routeIs('checkouts.cerrados') ? 'active' : '' }}" href="{{ route('checkouts.cerrados', request()->only('edificio_id')) }}">
            ✔ Cerrados
        </a>
    </ul>
    ...
    <div class="mb-3">
        ...
        <div class="filtro-scroll">
            ...
            {{-- TODOS --}}
            <a href="{{ request()->fullUrlWithQuery(['mes' => null]) }}" class="btn btn-sm me-2 flex-shrink-0 {{ request('mes') ? 'btn-outline-dark' : 'btn-dark' }}">
                📅 Todos
            </a>
            ...
            @php
            $activo = request('mes') == $numero;
            @endphp

            <a href="{{ request()->fullUrlWithQuery(['mes' => $numero]) }}" class="btn btn-sm me-2 flex-shrink-0" style="
    ...
    <div class="mb-3">
        ...
        {{-- 🔍 BUSCADOR --}}
        <div class="mb-2">
            <input type="text" id="buscadorEdificio" class="form-control form-control-sm" placeholder="🔍 Buscar edificio...">
        </div>
        ...
        <div class="filtro-scroll" id="listaEdificios">
            ...
            <div class="mb-3">
                <div class="d-flex overflow-auto pb-2" id="scrollEdificios">
                    ...
                    {{-- TODOS --}}
                    <a href="{{ request()->fullUrlWithQuery(['edificio_id' => null]) }}" class="btn btn-sm me-2 flex-shrink-0 {{ request('edificio_id') ? 'btn-outline-secondary' : 'btn-dark' }}">
                        Todos
                    </a>
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['edificio_id' => $e->id]) }}" class="btn btn-sm me-2 flex-shrink-0 {{ $activo ? 'activo' : '' }}" style="background-color: {{ $activo ? $color : 'transparent' }};border: 2px solid {{ $color }};color: {{ $activo ? '#fff' : $color }};">
                        {{ $e->nombre }}
                    </a>
                    @endforeach
            ...
            <tr>
                ...
                <td class="text-center">
                    <div class="d-flex justify-content-center gap-1">
                        @if ($c->pdf_solicitud)
                        <a href="{{ asset('checkout/' . $c->pdf_solicitud) }}" target="_blank" class="btn btn-sm btn-outline-primary" title="Solicitud">
                            📄
                        </a>
                        @endif

                        @if ($c->pdf_entrega)
                        <a href="{{ asset('checkout/' . $c->pdf_entrega) }}" target="_blank" class="btn btn-sm btn-outline-success" title="Entrega">
                            📄
                        </a>
                        @endif
                ...
                <td class="text-center">
                    ...
                    <form method="POST" action="{{ route('checkouts.estado', $c->id) }}">
                        ...
                        <select name="estado" class="form-select form-select-sm text-center border-{{ $color }} text-{{ $color }}" onchange="this.form.submit()">
                ...
                <td class="text-center">
                    ...
                    @if ($c->pdf_oc)
                    <a href="{{ asset('checkout/' . $c->pdf_oc) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-1 w-100">
                        📄 Ver OC
                    </a>
                    @endif
                ...
                <td class="text-center">
                    ...
                    @if ($c->pdf_factura)
                    <a href="{{ asset('checkout/' . $c->pdf_factura) }}" target="_blank" class="btn btn-sm btn-outline-success mt-1 w-100">
                        🧾 Ver Factura
                    </a>
                    @endif
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            ...
                            <li><a class="dropdown-item" href="{{ route('checkouts.historial', $c->id) }}">💬
                                    Historial</a></li>
                            <li><button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDocs{{ $c->id }}">📄 OC/Factura</button></li>
                            <li>
                                <hr class="dropdown-divider">
                            </li>
                            <li>
                                <form action="{{ route('checkouts.destroy', $c->id) }}" method="POST" onsubmit="return confirm('¿Seguro?')">
                                    @csrf @method('DELETE')
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($c->pdf_oc)
                                    <a href="{{ asset('checkout/' . $c->pdf_oc) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-2 w-100">
                                        Ver OC actual
                                    </a>
                                    @endif
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="nro_factura" class="form-control" value="{{ $c->nro_factura }}">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($c->pdf_factura)
                                    <a href="{{ asset('checkout/' . $c->pdf_factura) }}" target="_blank" class="btn btn-sm btn-outline-success mt-2 w-100">
                                        Ver Factura actual
                                    </a>
                                    @endif
                                </div>
